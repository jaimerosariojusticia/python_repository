...
#===============================================================#
COMMAND = 'show cable modem | include Cable'
...
for num in range(9, 1, -1):
    output = output.replace(';' * num,';').strip()

for line in output.split():
    line = line.strip()
    lmnt = line.split(';')

    CABLE = lmnt[0]; INTERFACE = lmnt[1]; PSID = lmnt[2]
    STAT = lmnt[3]; PID = lmnt[4]

    if STAT.__contains__("offline") == True:
        IPADDR = lmnt[5]; MACADDR = lmnt[6]
        remote_conn.send('clear cable modem offline ' + MACADDR + "\n")
        print("Cable modem " + MACADDR + " with status " + STAT + " cleared...")

    elif STAT.__contains__("reject") == True:
        PWR = lmnt[5]; IPADDR = lmnt[6]; MACADDR = lmnt[7]
        remote_conn.send('clear cable modem ' + MACADDR  + ' reset' + "\n")
        remote_conn.send('clear cable modem ' + MACADDR  + ' counters' + "\n")
        print("Resseting cable modem " + MACADDR + " with status " + STAT)
